...
here saying where it came from and under what terms.

**⚠ There are now THREE categories, not two, and the third is a real change to this file's own
rule.** It used to read "only CC0 or self-generated is acceptable". As of 2026-09-02 the game also
ships **third-party model output** — an image generated by a hosted service the project does not
run — and that is neither. It is not CC0: no such grant is made. It is not "self-generated"
either, and the phrase **"no third-party input" does not apply to it**: a hosted model is
third-party input by definition, whatever the prompt was. What governs it is the service's own
terms at the time of generation, which is what its rows record and what has to be re-checked
...
**⚠ The frog no longer ships from this model** — it was replaced on 2026-09-02 by a supplied
drawing. See "Third-party model output" below, which is a licence category this file did not
previously admit.
...
## The interface set (self-generated geometry)

`scripts/normalize-ui.py` draws these; nothing is downloaded and no model is involved. They are
**greyscale value maps** rather than artwork — a pixel's grey is its lightness ratio to the button's
face, and `src/ui/uiSprites.ts` colours them at runtime, which is what lets the primary button take
the active theme's accent. The geometry they are drawn to is `ART-STYLE.md`, measured off the two
reference sheets in `references/` (`ui-buttons-sheet.png`, `ui-screens-sheet.png`).

**⚠ The reference sheets themselves are third-party model output and are NOT shipped.** They live
in `references/`, which Vite does not copy, and they were used as a *measurement target*: what the
build reads is the table in `ART-STYLE.md`, not their pixels.

| File | Source | Licence | Added |
|---|---|---|---|
| `public/assets/ui/ui-btn{,-pressed}.png` | `scripts/normalize-ui.py` (this project) | self-generated | 2026-09-02 |
| `public/assets/ui/ui-round{,-pressed}.png` | `scripts/normalize-ui.py` (this project) | self-generated | 2026-09-02 |
| `public/assets/ui/ui-panel.png` | `scripts/normalize-ui.py` (this project) | self-generated | 2026-09-02 |
| `public/assets/ui/ui-bar-{frame,fill}.png` | `scripts/normalize-ui.py` (this project) | self-generated | 2026-09-02 |

A generation route was built and wired for these — `Remotion/src/scripts/snail_ui_prompts.py` and
`gen_snail_ui.py`, on DreamShaper XL, with `normalize-ui.py --audit` to measure what it returns.
**Nothing generated ships, and the probe did not return inside the session that built this**, so
no claim is made here about what that checkpoint produces for this style. The reason the set is
drawn rather than repaired is stated in `normalize-ui.py`'s docstring and does not depend on that
measurement: for a style specified as a flat face, a band and a lip at stated fractions, an even
contour and one radius, every pixel is implied.

## Third-party model output — NOT CC0, NOT self-generated

**The one category in this file whose licence is a service's terms rather than a grant.** Read
the amended note at the top: these rows exist because the game ships them, not because they are
cleared the way everything above is. Before submission each one needs its terms re-checked, and
the safe fallback for any of them is the asset it replaced — which is why the row it supersedes is
struck through above rather than deleted.

| File | Source | Prompted by | Licence | Added |
|---|---|---|---|---|
| `public/assets/critter/critter-frog-{0,1}.png` | Google **Gemini** (hosted image model), delivered as `references/frog 1.jpg` | the project owner | **Gemini / Google service terms — NOT CC0** | 2026-09-02 |
| `public/assets/critter/critter-frog-air.png` | Google **Gemini** (hosted image model), delivered as `references/frog 4.jpg` | the project owner | **Gemini / Google service terms — NOT CC0** | 2026-09-02 |

Two further facts about this one, both of which belong with the provenance rather than in a code
comment:

- **Two of the four supplied images ship.** `frog 1.jpg` is the crouch and `frog 4.jpg` the tucked
  airborne pose. `frog 2.jpg` and `frog 3.jpg` are reference only and are not in `public/`: all
  three of the first batch place the head identically and vary only the legs, so they do not work
  as phases of a jump. The hop is built by deforming the crouch and swapping to the tuck at the top
  of the arc — see `src/run/critterJump.ts`.
- **What ships is not what was supplied.** `scripts/normalize-critter.py` keys the baked
  checkerboard out, strips the halo, and **rebuilds the outer contour** to one thickness; the
  delivered file is a derivative. That does not change its licence position.
